...
dict_simb = {'EURUSD':'venta','USDJPY':'compra','USDCAD':'venta','GBPJPY':'ambos','GBPUSD':'ambos'}
dict_simb2 = {'.US500Cash':'compra','.DE40Cash':'ambos','GBPNZD':'venta'}

def anomaly_ma_bot(diccionario_symb,timeframe,num_velas,lotaje,mean_window,sigma):
    for simb,accion in diccionario_symb.items():
        datos = extraer_datos(simb,num_velas,timeframe)
        datos['Ma'] = datos['close'].rolling(mean_window).mean()
        datos['dif_ma'] = datos['close'] - datos['Ma']
        datos['asceleracion_ma'] = datos['Ma'] - datos['Ma'].shift()
        ...
        tp = (last_price + datos['Ma'].iloc[-1] )/2

        if (accion == 'compra') or (accion == 'ambos'):
            if last_price < lim_inf:
                enviar_operaciones(simb,mt5.ORDER_TYPE_BUY,lotaje,tp)
        elif (accion == 'venta') or (accion == 'ambos'):
            if last_price > lim_sup:
                enviar_operaciones(simb,mt5.ORDER_TYPE_SELL,lotaje,tp)
        else:
             print(f'Existe un error en el tipo de acción del símbolo {accion}')

while True:
    if datetime.datetime.now().minute == 0:
        anomaly_ma_bot(dict_simb,mt5.TIMEFRAME_H1,4000,0.01,20,3)
        anomaly_ma_bot(dict_simb2,mt5.TIMEFRAME_M15,3000,0.01,100,3.5)
