...
def pivot_point_bot(symbol,server_hours,vol):
    ...
    hoy = datetime.datetime.now() #Hora actual del sistema del computador local
    ...
    enviar_operaciones_pendientes(symbol,mt5.ORDER_TYPE_BUY_LIMIT,f_support,vol,timestamp,tp_limit,s_support)
    enviar_operaciones_pendientes(symbol,mt5.ORDER_TYPE_SELL_STOP,s_support,vol,timestamp,t_support,f_support)
    enviar_operaciones_pendientes(symbol,mt5.ORDER_TYPE_SELL_LIMIT,f_resistance,vol,timestamp,tp_limit,s_resistance)
    enviar_operaciones_pendientes(symbol,mt5.ORDER_TYPE_BUY_STOP,s_resistance,vol,timestamp,t_resistance,s_resistance)

while True:
    list_simbs = ['XAUUSD','EURUSD','USDJPY','USDCAD','GBPJPY','GBPUSD','GBPNZD','EURAUD']
    for symb in list_simbs:
        pivot_point_bot(symb,7,0.05)
