...
def bollinger_bands_strategy(symbol,lot_size,timeframe,max_trades,length_bb,lower_std,upper_std,pips_tp,pips_sl):
    ...
    if (last_price >= lim_sup) and num_trades < max_trades:
        ...
        send_trade(symbol,lot_size,mt5.ORDER_TYPE_SELL,take_profit,stop_loss)
    elif (last_price <= lim_low) and num_trades < max_trades:
        take_profit = last_price + pips_tp*pips_symbol
        stop_loss = last_price - pips_sl*pips_symbol
        send_trade(symbol,lot_size,mt5.ORDER_TYPE_BUY,take_profit,stop_loss)
    else:
